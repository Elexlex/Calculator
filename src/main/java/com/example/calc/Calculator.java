...
public class Calculator extends Application {
    ...
    private enum Op {NOOP, ADD, SUBTRACT, MULTIPLY, DIVIDE, SIN, COS, TAN, LOG, SQRT, SQUARE}
    ...
    private TextField createScreen() {
        ...
        screen.textProperty().bind(Bindings.format("%.4f", value));
        return screen;
    }
    ...
    private ObjectProperty<Op> determineOperand(String s) {
        ...
        switch (s) {
            ...
            case "sin" -> triggerOp.set(Op.SIN);
            case "cos" -> triggerOp.set(Op.COS);
            case "tan" -> triggerOp.set(Op.TAN);
            case "log" -> triggerOp.set(Op.LOG);
            case "sqrt" -> triggerOp.set(Op.SQRT);
            case "x^2" -> triggerOp.set(Op.SQUARE);
        }
        return triggerOp;
    }
    ...
    private void makeEqualsButton(Button button) {
        ...
        button.setOnAction(actionEvent -> {
            switch (stackOp) {
                ...
                case SIN -> value.set(Math.sin(Math.toRadians(value.get())));
                case COS -> value.set(Math.cos(Math.toRadians(value.get())));
                case TAN -> value.set(Math.tan(Math.toRadians(value.get())));
                case LOG -> value.set(Math.log(value.get()));
                case SQRT -> value.set(Math.sqrt(value.get()));
                case SQUARE -> value.set(Math.pow(value.get(), 2));
            }
        });
    }
}
